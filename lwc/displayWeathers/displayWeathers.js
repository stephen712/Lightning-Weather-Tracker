...
export default class DisplayWeathers extends LightningElement {
    ...
    wiredWeatherData({ error, data }) {
        console.log('here11');
        if (data) {
            this.weatherData = data.map(cityWeatherMap => {
                const city = Object.keys(cityWeatherMap)[0]; // Extract the city key
                const weather = cityWeatherMap[city]; // Extract the weather for the city
            //    console.log({city, weather}); // Log each city and weather
                return {
                    city,
                    weather,
                };
            });
            this.datesAndWeatherData = this.extractDatesAndWeatherCodes(this.weatherData); // Call the function with weatherData
            console.log('here2'+this.datesAndWeatherData);
           console.log(this.weatherData[0].city);
    ...
    handleBillingCityChange(event) {
        ...
        console.log('inside handleBillingCityChange');
        this.showWeatherData = true;
    }
    extractDatesAndWeatherCodes(weatherData) {
        console.log('inside extractDatesAndWeatherCodes');
      ...
      "8000": "Thunderstorm"
        ...
        if (weatherData && weatherData.length > 0) {
            ...
            return weatherData
                .filter(cityWeather => cityWeather.weather) // Filter out items without weather data
                .map(cityWeather => {
                    const city = cityWeather.city;
                    console.log(city);
                    const weather = JSON.parse(this.htmlDecode(cityWeather.weather));
                    console.log(weather);
    
                    const timelines = weather.data.timelines[0];
                    console.log('timelines' + timelines);
    
                    if (timelines && timelines.intervals) {
                        const datesandWeathers = timelines.intervals.map(interval => {
                            const date = interval.startTime;
                            ...
                            return { city, date, weatherDescription };
                        });
                        return datesandWeathers;
                    } else {
                        return [];
                    }
                })
    ...
    handleShowWeatherData() {
        ...
        console.log('inside handleShowWeatherData');
        console.log(this.billingCity);
        this.showButton = true;
        this.filteredData = this.datesAndWeatherData.filter(item => item.city == this.billingCity);
